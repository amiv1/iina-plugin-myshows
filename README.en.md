...
- **Network Request**: Used to make requests towards the MyShows API (authentication and episode management)
- **Show OSD**: To show when an episode has been marked as watched, if enabled in the preferences

## How It Works

1. The plugin authenticates directly with MyShows by posting credentials to `https://myshows.me/api/session` and obtaining a bearer token.
